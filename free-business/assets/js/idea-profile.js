document.addEventListener('DOMContentLoaded', function() {
    //get data from google auth
   

    const photo = {
        photo: "data:image/png;base64,",
    };
    const userData = sessionStorage.getItem('userData');
    const user = JSON.parse(userData);
    const profileImageElement = document.getElementById('profileImage');

        if (user.photo && user.photo !== "") {
          // If 'user.photo' is not empty, set the 'src' attribute with the base64 image data
          //  profileImageElement.src = user.photo;
          profileImageElement.src = `data:image/png;base64,${user.photo}`;

      } else {
          // If 'user.photo' is empty or undefined, set a default placeholder image or do not display an image
          // Example: Set a placeholder image URL
          const placeholderImageUrl = photo.photo;
          profileImageElement.src = placeholderImageUrl;


      }
    // Retrieve user data from sessionStorage
    const ideaData = sessionStorage.getItem('ideaData');
    

    //console.log('Idea Data:', ideaData);

   
      // Check if businessData is valid JSON
let business;
try {
  business = JSON.parse(ideaData);
} catch (error) {
  console.error("Error parsing businessData:", error);
}

// Display user data on the page as needed
const profNameElement = document.getElementById('ideaProfName');
const profCountryElement = document.getElementById('ideaProfCountry');
const profSummaryElement = document.getElementById('ideaProfSummary');
const profMarketElement = document.getElementById('ideaProfMarket');
const profTractionElement = document.getElementById('ideaProfTraction');
const profCategoryElement = document.getElementById('ideaProfCategory');
const profFoundersElement = document.getElementById('ideaProfFounders');
const profUserElement = document.getElementById('ideaUser');
const profEmailElement = document.getElementById('ideaEmail');
const profPhoneElement = document.getElementById('ideaPhone');


// Check if business data is available before trying to display it
if (business) {
  profNameElement.textContent = business.name || '';
  profCountryElement.textContent = business.nationality || '';
  profSummaryElement.textContent = business.summary || '';
  profMarketElement.textContent = business.market || '';
  profTractionElement.textContent = business.traction || '';
  profFoundersElement.textContent = business.founders || '';
  profCategoryElement.textContent = business.category || '';
  profUserElement.textContent = business.username || '';
  profEmailElement.textContent = business.useremail || '';
  profPhoneElement.textContent = business.userphone || '';

  
        // Get the base64 image data from the 'photo' property
        // Assuming 'user.photo' contains the base64 image data
       
        const logo = document.getElementById('ideaProfLogo');
       
        if (business.photo && business.photo !== "") {
            // If 'user.photo' is not empty, set the 'src' attribute with the base64 image data
            // profileImageElement.src = user.photo;
            
            logo.src = `data:image/png;base64,${business.photo}`;
        } else {
            // If 'user.photo' is empty or undefined, set a default placeholder image or do not display an image
            // Example: Set a placeholder image URL
            const placeholderImageUrl = photo.photo;
            
            logo.src = placeholderImageUrl;

        }
} else {
  console.error("Business data is not available.");
  // Optionally, you can display a default message or take other actions
}
        
      
       


       
    

        //profileImageElement.src = `data:image/png;base64,${base64ImageData}`;
        // Display other user information as needed
    
});