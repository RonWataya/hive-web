document.addEventListener('DOMContentLoaded', function() {
    //get data from google auth
    
    const photo = {
        photo: "data:image/png;base64,",
    };
    const userData = sessionStorage.getItem('userData');
    const user = JSON.parse(userData);
    const profileImageElement = document.getElementById('profileImage');

        if (user.photo && user.photo !== "") {
          // If 'user.photo' is not empty, set the 'src' attribute with the base64 image data
          //  profileImageElement.src = user.photo;
          profileImageElement.src = `data:image/png;base64,${user.photo}`;

      } else {
          // If 'user.photo' is empty or undefined, set a default placeholder image or do not display an image
          // Example: Set a placeholder image URL
          const placeholderImageUrl = photo.photo;
          profileImageElement.src = placeholderImageUrl;


      }
    // Retrieve user data from sessionStorage
    const businessData = sessionStorage.getItem('businessData');
    

    //console.log('Business Data:', businessData);

   
      // Check if businessData is valid JSON
let business;
try {
  business = JSON.parse(businessData);
} catch (error) {
  console.error("Error parsing businessData:", error);
}

// Display user data on the page as needed
const profNameElement = document.getElementById('businessProfName');
const profCountryElement = document.getElementById('businessProfCountry');
const profAboutElement = document.getElementById('businessProfAbout');
const profTermsElement = document.getElementById('businessProfTerms');
const profNoteElement = document.getElementById('businessProfNote');
const profRatesElement = document.getElementById('businessProfRates');
const profCategoryElement = document.getElementById('businessProfCategory');
const profAddressElement = document.getElementById('businessProfAddress');
const profCityElement = document.getElementById('businessProfCity');
const profWhatsappElement = document.getElementById('whatsappProfContact');
const profEmailElement = document.getElementById('businessProfEmail');
const profPhoneElement = document.getElementById('businessprofPhone');

// Check if business data is available before trying to display it
if (business) {
  profNameElement.textContent = business.name || '';
  profCountryElement.textContent = business.country || '';
  profAboutElement.textContent = business.about || '';
  profTermsElement.textContent = business.terms || '';
  profNoteElement.textContent = business.note || '';
  profRatesElement.textContent = business.rates || '';
  profCategoryElement.textContent = business.category || '';
  profAddressElement.textContent = business.address || '';
  profCityElement.textContent = business.city || '';
  profWhatsappElement.textContent = business.whatsapp || '';
  profEmailElement.textContent = business.email || '';
  profPhoneElement.textContent = business.phone || '';

  
        // Get the base64 image data from the 'photo' property
        // Assuming 'user.photo' contains the base64 image data
       
        const logo = document.getElementById('businessProfLogo');
        

        if (business.photo_data && business.photo_data !== "") {
            // If 'user.photo' is not empty, set the 'src' attribute with the base64 image data
            // profileImageElement.src = user.photo;
            
            logo.src = `data:image/png;base64,${business.photo_data}`;
        } else {
            // If 'user.photo' is empty or undefined, set a default placeholder image or do not display an image
            // Example: Set a placeholder image URL
            const placeholderImageUrl = photo.photo;
            
            logo.src = placeholderImageUrl;

        }
} else {
  console.error("Business data is not available.");
  // Optionally, you can display a default message or take other actions
}
        
      
       


       
    

        //profileImageElement.src = `data:image/png;base64,${base64ImageData}`;
        // Display other user information as needed
    
});