...
function fetchCompanies(companyList, city, country, category) {
    fetch('https://moneyhive-mw.com:2000/api/portifolio')
        ...
        .then(data => {
            ...
            const maxProfiles = 6; // Set the maximum number of profiles to display
            ...
            const filteredCompanies = data.filter(company => {
                ...
                const matchCountry = country ? company.country.toLowerCase().includes(country.toLowerCase()) : true;
            ...
            let displayedProfiles = 0; // Counter for displayed profiles

            filteredCompanies.forEach(company => {
                if (company.category === "Investor") {
                    return;
                }

                if (displayedProfiles >= maxProfiles) {
                    return; // Break out of the loop if the limit is reached
                }
                ...
                companyProfile.innerHTML = `
                    <div class="company_profile_info" id="${company.name}">
                        ...
                        <img src="${company.photo_data ? `data:image/png;base64, ${company.photo_data}` : 'assets/img/hive-logo.png'}" alt="Logo">
                            <h3>${company.name}</h3>
                            <h4>${company.note}</h4>
                            <ul>
                                <li><a href="#" title="" class="follow">${company.rates}</a></li>
                ...
                displayedProfiles++; // Increment the displayed profiles counter
            });
        })
        .catch(error => {
...
function portifolio(
    ...
    if (companyData) {
        ...
        document.getElementById(companyterms).textContent = company.terms;
        document.getElementById(companyrates).textContent = company.rates;
        document.getElementById(companyabout).textContent = company.about;
        document.getElementById(companycategory).textContent = company.category;
        document.getElementById(companycountry).textContent = company.country;
        ...
        const profileLogo = document.getElementById('logoImage');
        profileLogo.src = `data:image/png;base64,${company.photo_data}`;
        // Set the data-company-id attribute for the review button
        reviewButton.setAttribute('data-company-id', company.id);
    }
}
...
portifolio("companyName", "companyTerms", "companyRates", "companyAbout", "companyCategory", "companyCountry", "companyCity", "companyAddress");